...
class BiMpmMatching(nn.Module):
    def __init__(self,
        ...
        if with_full_match:
            ...
            output_dim += (num_perspectives + 1) * 1
...
class GatedMultiHeadAttn(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, hidden_dim, num_head,
        ...
        self.o_net = nn.Linear(query_dim, hidden_dim, bias=False)
        self.g_net = nn.Linear(2 * hidden_dim, hidden_dim, bias=True)
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        ##### linear projection
        # attn_out = self.o_net(attn_vec)
        attn_query = self.o_net(query)

        ##### gate
        # gate = F.sigmoid(self.g_net(torch.cat([query, attn_out], dim=2)))
        # attn_out = gate * value + (1 - gate) * attn_out
        gate = F.sigmoid(self.g_net(torch.cat([attn_query, attn_vec], dim=2)))
        attn_out = gate * attn_query + (1 - gate) * attn_vec
...
class GatedMultiHeadAttn2(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, hidden_dim, num_head,
                 dropatt=0.0,
                 act_func="softmax", add_zero_attn=False,
                 pre_lnorm=False, post_lnorm=False):
        super(GatedMultiHeadAttn2, self).__init__()
        assert hidden_dim % num_head == 0
