...
def train(train_data,test_data,pdtb_exp_data,data_name,model, model_fn,
    ...
    logger.info("loss_ff = loss_imp + alpha * KL_loss Pre-trained_2tasks_2datasets BERT-BASE alpha")
        ...
        for batch in batch_generator(train_arg1_info,train_arg2_info,train_Y,batch_size, train_connect=None):
            ...
            if(step%50==0):
                ...
                if test_f1 > best_f1:
                    ...
                    # np.savez(data_dir + "OTMT_predict_teacher4.npz", predict=predict1, target=target1)
...
def run(domain, data_dir, model_dir, valid_split, runs, epochs, lr, dropout, class_num, batch_size=16, alpha=0.0001):
    ...
    for r in range(runs):
        filename = 'model_exp/Pre-trained_2tasks_2datasets_4_bert_base.pt'
        # print(filename)
        ...
        model.cls_classifier_e.load_state_dict(checkpoint['cls_class_4'])
        model.con_classifier_e.load_state_dict(checkpoint['con_class_4'])
...
if __name__ == "__main__":
    ...
    if not os.path.exists('./log'):
        os.mkdir('./log')
    log_file = './log/Pre-trained_2tasks_2datasets_KL_loss_alpha_bert_base-{}-{}-{}-{}-{}.log'.format(args.class_num, strftime("%y%m%d-%H%M%S", localtime()), args.seed, args.batch_size, args.alpha)
